<template>
	<view :class="['digital-keyboard',{'keyboard-show':!keyBoardSwitch}]">
		<view class="flex justify-end padding-sm bg-white" style="width: 100%;">
			<text @click="Close" class="margin-right padding-sm">完成</text>
		</view>
		<view class="list"  
		@click="setKey(key)"  hover-class="key-hove" 
		v-for="(key,index) in data" :key='index'>
			{{ key == 'b' ? '' : key=='.'&&!decimalPoint?'':key}}
		</view>
	</view>
</template>
