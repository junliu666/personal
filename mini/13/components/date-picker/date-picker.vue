<template>
	<view>
		<view class="order-time" @tap="showCalendar()" v-if="!modal" >
			<view class="time-viewer" v-if="singleDate">开始日期 {{ choiceDate[0].year }}年{{ choiceDate[0].month }}月{{ choiceDate[0].day }}日</view>
			<view class="time-viewer" v-else>
				<text class="goInHotel">开始日期</text>
				...
				<text class="left-hotel">结束日期</text>
						...
						<view v-for="(monthData, index) in date" :key="index" class="month">
							...
							<view class="month-content">
								...
								>
									...
									<view class="beginTip" v-if="choice === false && !singleDate">请选择结束日期</view>
