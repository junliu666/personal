<template>
	<view :class=" viewReply ? 'main bg-white' : ''">
		<cu-custom bgColor="bg-main" :isBack="true"><text slot="backText" style="font-size: 30upx; margin-left: 20upx;"></text><block slot="content">意见反馈</block><text slot="right"  @tap="ViewReply" class="margin-right" style="padding: 20upx">{{ viewReply ? '查看回复' : '新的反馈' }}</text></cu-custom>
		...
		<view class="" v-if="viewReply">
			...
			<view class="flex align-center flex-wrap" style="width: 680upx; margin-left: 35upx;">
				<text style="margin: 0; margin-right: 20upx; margin-top: 20upx;" :class=" item.status == false ? 'bg-gray cu-tag radius  ' : ' bg-main  cu-tag radius'" v-for="(item, index) in typeList">
					{{ item.title }}
				</text>
			</view>
		...
		<view class="cu-bar tabbar bg-foot shadow foot" style="box-shadow: none;">
			<view class="action"  data-cur="basics" style="height: 100%; width: 60%;"> 
				<button @tap="Post" style="width: 60%; height: 90%; font-size: 40upx;"  class="cu-btn round bg-main">提交反馈</button>
			</view>
		</view>
		</view>
				...
				<view class="">
					<view style="width: 640upx;" class="flex justify-between text-lg text-black">
						<view class="flex align-center">
							<text class="margin-right-sm cu-tag sm bg-main radius">已回复</text>
							<text>交易纠纷</text>
