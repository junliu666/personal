<template>
	<view style="min-height: 100vh; background-color: white;">
		<cu-custom bgColor="bg-transparent" :isBack="true"><text slot="backText" style="font-size: 30upx; margin-left: 20upx;"></text><block slot="content"></block><text slot="right" @tap="Edit"  class="margin-right" style="padding: 20upx"></text></cu-custom>
		...
		<scroll-view id="scroll"  scroll-x="true" class="  padding-tb-sm" style="white-space: nowrap; width: 100vw; z-index: 9999;">
			<view :id="'nav-'+index" @click="ChangeNav(index)" class=" " :class=" index == tabCur ? ' text-xlg text-bold text-main' : 'text-lg'" style="margin-left: 35upx; display: inline-block;" :key="index" v-for="(item, index) in nav">
				{{ item }}
			</view>
			<view class="round bg-main margin-top-xs" :style="'transform:translateX(' + (preLength + scrollLeft )  + 'upx); width: 80upx; height: 10upx; transition: 0.5s'"></view>
