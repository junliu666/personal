	...
	<view>
		...
		<view class="cu-bar bg-white search fixed" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				...
				<input type="text" placeholder="搜索好友" confirm-type="search"></input>
			</view>
			<view class="action">
				<button class="cu-btn bg-main round">搜索</button>
			</view>
		</view>
				...
				<view :class="'indexItem-' + item.name" :id="'indexes-' + item.name" :data-index="item.name">
					...
					<view class="cu-list menu-avatar no-padding">
						<view class="cu-item" v-for="(items,sub) in 3" :key="sub">
							...
							<view class="content">
								<view class="text-black">{{item.name}}<text class="text-abc">{{list[sub].name}}</text>君</view>
								<view class="text-gray text-sm">
									{{ sub == 0 ? '通过搜索好友添加' : sub == 1 ? '通过发布任务添加' : '通过接零活添加' }}
								</view>
							</view>
						</view>
					</view>
