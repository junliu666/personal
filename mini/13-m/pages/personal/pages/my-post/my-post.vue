				...
				<view @click="GoDetail" class="padding-sm margin-top bg-white radius" style="border-radius: 25upx; width: 680upx; margin-left: 35upx;">
					...
					<view class="" v-if="item.status == 1 || item.status == 7">
						...
						<view class="margin-top-sm padding-tb-sm align-center flex justify-between" style="width: 100%; border-top: 1px solid #F5F5F5;">
							...
							<text class="text-main text-bold text-lg">{{ item.status == 1 || item.status == 2 ? '总预付金' : '总支付金额' }}{{ item.insuranceRequire ? item.totalPrice + item.price : item.totalPrice }}元</text>
						</view>
					</view>
					...
					<view :class="item.status == 0 ?  'margin-top-xs flex justify-end' : 'margin-top flex justify-end'" style="width: 100%;">
						...
						<button v-if="item.status != 5 && item.status != 6 && item.status != 7" class="line-gray cu-btn round margin-left-sm">取消任务</button>
						<button v-if="item.status == 1 " class="line-gray cu-btn round margin-left-sm">修改信息</button>
						<button v-if="item.status == 2 " class="line-gray cu-btn round margin-left-sm">提前开始</button>
						<button v-if="item.status == 1" class="bg-main cu-btn round margin-left-sm">预付</button>
						<button v-if="item.status == 2" class="bg-main cu-btn round margin-left-sm">选择接单者</button>
						<button @click.stop="GoDoingDetail" v-if="item.status == 4 || item.status == 5 || item.status == 6" class="line-gray cu-btn round margin-left-sm">任务进程</button>
						<button v-if="item.status == 4" class="bg-main cu-btn round margin-left-sm">任务完成</button>
						<button v-if="item.status == 5" class="bg-main cu-btn round margin-left-sm">确认尾款</button>
						
						<button v-if="item.status == 6" class="bg-main cu-btn round margin-left-sm">评价</button>
						<button v-if="item.status == 7" class="bg-main cu-btn round margin-left-sm">退款进度</button>
					</view>
